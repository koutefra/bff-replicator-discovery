...
constexpr size_t kNumTerminationReasons = 3;

enum TerminationReason : uint8_t {
  kTerminationStepCap = 0,
  kTerminationIpOutOfBounds = 1,
  kTerminationBracketMismatch = 2,
};
...
struct SimulationParams {
  ...
  bool structured_partner_interaction = false;
  bool focal_analysis = false;
  size_t focal_program_count = 0;
  std::vector<uint64_t> init_byte_cdf;
  std::vector<uint8_t> structured_partner_pool;
...
struct SimulationState {
  ...
  size_t count_op_lt;
  size_t count_op_gt;
  size_t count_op_lbrace;
  size_t count_op_rbrace;
  size_t count_op_plus;
  size_t count_op_minus;
  size_t count_op_dot;
  size_t count_op_comma;
  size_t count_op_lbracket;
  size_t count_op_rbracket;
  size_t count_zero;
  float mean_step_count_per_interaction;
  float frac_term_step_cap;
  float frac_term_ip_out_of_bounds;
  float frac_term_bracket_mismatch;
  ...
  bool has_hamming_stats = false;
  float mean_hamming = 0.0f;
  float median_hamming = 0.0f;
  float p90_hamming = 0.0f;
