...
struct Bff {
  ...
  static __device__ size_t Evaluate(
      uint8_t *tape, size_t stepcount, bool debug,
      size_t *executed_steps = nullptr,
      uint8_t *termination_reason = nullptr) {
    size_t nskip = 0;

    int pos = 0;
    int head0_pos = 0;
    int head1_pos = 0;
    uint8_t reason = kTerminationStepCap;
    ...
    for (; i < stepcount; i++) {
      ...
      bool bracket_mismatch = false;
      if (debug) {
        PrintProgramInternal(head0_pos, head1_pos, pos, tape,
                             2 * kSingleTapeSize, nullptr, 0);
      }
      if (!EvaluateOne(tape, head0_pos, head1_pos, pos, bracket_mismatch)) {
        nskip++;
      }
      if (pos < 0 || pos >= 2 * kSingleTapeSize) {
        reason = bracket_mismatch ? kTerminationBracketMismatch
                                  : kTerminationIpOutOfBounds;
        i++;
        break;
      }
      pos++;
      if (pos >= 2 * kSingleTapeSize) {
        reason = kTerminationIpOutOfBounds;
        i++;
        break;
      }
    }
